...
export function Dashboard() {
  ...
  const statCards = [
    ...(isAdmin() ? [{ label: 'Usuarios', value: stats.usuarios, icon: Users, color: 'text-blue-600 bg-blue-50', to: '/usuarios' }] : []),
    { label: 'Anuncios', value: stats.anuncios, icon: Megaphone, color: 'text-amber-600 bg-amber-50', to: '/anuncios' },
    { label: 'Reservas activas', value: stats.reservas, icon: CalendarRange, color: 'text-green-600 bg-green-50', to: '/reservas' },
    { label: 'Horarios', value: stats.horarios, icon: Calendar, color: 'text-purple-600 bg-purple-50', to: '/horarios' },
  ]
      ...
      <div className="grid lg:grid-cols-2 gap-6">
          ...
          {recentAnuncios.length === 0 ? (
            ...
            <div className="space-y-3">
              {recentAnuncios.map(a => (
                <Link key={a.id} to={`/anuncios/${a.id}`} className="block p-3 rounded-lg hover:bg-gray-50 border border-gray-100">
                  <p className="font-medium text-sm text-gray-900">{a.titulo}</p>
                  <p className="text-xs text-gray-500 mt-1">{formatDateTime(a.fechaPublicacion)}</p>
                </Link>
              ))}
            </div>
          )}
          ...
          {misHorarios.length === 0 ? (
            ...
            <div className="space-y-2">
              {misHorarios.slice(0, 5).map(h => {
                ...
                return (
                  <div key={h.id} className="flex items-center justify-between p-2 rounded bg-gray-50 text-sm">
                    <span className="font-medium">{dias[h.diaSemana]}</span>
                    <span className="text-gray-500">{h.horaInicio} - {h.horaFin}</span>
                  </div>
                )
              })}
            </div>
        ...
        {isAdmin() && pendientes > 0 && (
          <Card className="lg:col-span-2">
            <Link to="/horarios/cambios" className="flex items-center gap-3 p-3 rounded-lg bg-amber-50 hover:bg-amber-100 transition-colors">
              ...
              <div>
                ...
                <p className="text-xs text-amber-600">Haz clic para revisar</p>
              </div>
            </Link>
          </Card>
        )}
